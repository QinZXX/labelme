...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        self.labelList.itemChanged.connect(self.labelItemChanged) # 自定义信号即self.model().itemChanged 这里self代表labelList
        # 这个labelList类是自定义,继承自QtWidgets.QListView
        self.labelList.itemDropped.connect(self.labelOrderChanged) # 不是应该没有参数吗 槽函数。。。item是什么，信号?
        ...
        self.fileSearch.textChanged.connect(self.fileSearchChanged) # 文件搜索目录改变的 信号
        ...
        self.canvas = self.labelList.canvas = Canvas( # python可以连续赋值
            epsilon=self._config['epsilon'], # 数学参数 伊普洗龙
            double_click=self._config['canvas']['double_click'], # 双击
        ) ### 画布 （显示和绘图的空间）
        # 通过此canvas值来保存文件，信息均在这个类里面。同时也等于labelList.canvas  这个是新增的类成员，定义里不包含
        self.canvas.zoomRequest.connect(self.zoomRequest) # 连接窗口放缩槽函数

        scrollArea = QtWidgets.QScrollArea() # 滚动区域
        scrollArea.setWidget(self.canvas) # canvas设为滚动区域的子控件
        ...
        self.canvas.newShape.connect(self.newShape) # newShape
        self.canvas.shapeMoved.connect(self.setDirty) # shapeMove 不管（时间改变）
        self.canvas.selectionChanged.connect(self.shapeSelectionChanged) # 选择改变
        self.canvas.drawingPolygon.connect(self.toggleDrawingSensitive) # 画出多边形

        self.setCentralWidget(scrollArea) # 中心控件
        ...
        # Actions
        action = functools.partial(utils.newAction, self) # self as 1st param
        ...
        createRectangleMode = action(
            self.tr('Create Rectangle'), #############显示内容
            lambda: self.toggleDrawMode(False, createMode='rectangle'), # 槽函数
            shortcuts['create_rectangle'],# 快捷方式
            'objects', # 图标
            self.tr('Start drawing rectangles'), # 提示信息
            enabled=False, # 使能
        )
        ...
        # Callbacks:
        self.zoomWidget.valueChanged.connect(self.paintCanvas) # zoom改变=>绘制画布

        self.populateModeActions() # 居住于（构成）动作模式

        # self.firstStart = True
        # if self.firstStart:
        #    QWhatsThis.enterWhatsThisMode()

    def menu(self, title, actions=None): # 添加title菜单和动作
    ...
    def noShapes(self): # 是否没有标注结果，标注结果保存在labelList中
        return not len(self.labelList)

    def populateModeActions(self): # 构成动作模式
    ...
    def canvasShapeEdgeSelected(self, selected, shape): # 绘制
        self.actions.addPointToEdge.setEnabled(
            selected and shape and shape.canAddPoint()
        )

    def queueEvent(self, function): # 队列事
    ...
    def currentItem(self):  ###### 当前的labelList中的Items  选择则为真，返回第一个item
        items = self.labelList.selectedItems()
        if items:
            return items[0]
        return None

    def addRecentFile(self, filename): # 添加最近打开文件
    ...
    def toggleDrawMode(self, edit=True, createMode='polygon'):
        self.canvas.setEditing(edit) # 设置处于绘制状态
        self.canvas.createMode = createMode # 绘制模式
        if edit:
            self.actions.createMode.setEnabled(True) # 设置控件可见 （可点击）  后面通过repaint重绘
            self.actions.createRectangleMode.setEnabled(True)
            self.actions.createCircleMode.setEnabled(True)
            self.actions.createLineMode.setEnabled(True)
            self.actions.createPointMode.setEnabled(True)
    ...
    def fileSearchChanged(self): # 文件搜索目录改变
    ...
    def shapeSelectionChanged(self, selected_shapes): # 形状选择
    ...
    def loadShapes(self, shapes, replace=True): # 加载shapes到labelList中？
        self._noSelectionSlot = True
        for shape in shapes:
            self.addLabel(shape) # 把每个标注加到标注列表中
        self.labelList.clearSelection()
        self._noSelectionSlot = False
    ...
    def loadLabels(self, shapes):
        s = []
        for shape in shapes: # shape为一个标注，比如矩形
            label = shape['label']
            points = shape['points']  # 从shapes获取points，对矩形来说是两个点
            ...
            for x, y in points: ######################这是加载points 的地方，生成points的接口在何处，用什么数据结构保存的
                shape.addPoint(QtCore.QPointF(x, y)) # 这是qt的点类型  不能更改这个类。。 所以把Shape的方法全都修改一遍
            # for x, y,t in points: ## 修改
            #     shape.addPoint(QtCore.QPointF(x, y),t) # addPoint均要修改
            shape.close()
    ...
    def saveLabels(self, filename):  #########
        ...
        def format_shape(s):
            data = s.other_data.copy()  # 拷贝其他信息来更新
            data.update(dict(
                label=s.label.encode('utf-8') if PY2 else s.label,
                # points=[(p.x(), p.y()) for p in s.points], # points知道了，坐标的列表，p 这个坐标的定义在哪  这里是元组，可是json中是list，后面还有更改
                pointsNew=[(p[1], p[2],p[3]) for p in s.pointsNew], # 保存的
                group_id=s.group_id,
        ...
        shapes = [format_shape(item.shape()) for item in self.labelList] # 列表 （元素为字典） item就是一个shape
        flags = {}
    ...
    def labelItemChanged(self, item):
        shape = item.shape() # 获取形状
        self.canvas.setShapeVisible(shape, item.checkState() == Qt.Checked) # 改变可视化形状

    def labelOrderChanged(self): # 标签顺序改变
        self.setDirty() # 设置为添加绘制模型
        self.canvas.loadShapes([item.shape() for item in self.labelList]) # 加载形状
        # 这里的item是labelList的item
    ...
    def togglePolygons(self, value): # 触发多边形
        for item in self.labelList:
            item.setCheckState(Qt.Checked if value else Qt.Unchecked)

    def loadFile(self, filename=None): # 加载文件
        ...
        if self.labelFile:
            self.loadLabels(self.labelFile.shapes) # 这里是shape，这是加载方法，我要看shapes怎么来的，肯定有个保存的函数
            # 然后鼠标的信号时间连接到不对，应该是存在某些临时变量里面的，点击保存后再把临时变量加载到函数
            # 临时变量的什么位置，在哪找到他们
            if self.labelFile.flags is not None:
                flags.update(self.labelFile.flags) # 更新flags
